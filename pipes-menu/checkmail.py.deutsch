...
class Mails(object):
    ...
    def __xmlSegment(self, root, mail_num, value_list):
        ...
        setread_entry = ET.SubElement(menu, "item", attrib = {"label":u"gelesen"})
        ...
        delete_menu = ET.SubElement(menu, "menu", attrib = {"id":str(mail_num) + "-delete-menu","label":u"löschen"})
        delete_entry = ET.SubElement(delete_menu, "item", attrib = {"label":u"bestätigen"})
    ...
    def __pipeMenu(self, data_dict, state):
        ...
        if len(data_dict) == 0:
            empty_entry = ET.SubElement(root, "item", attrib = {"label":"keine neuen mails"})
        ...
        refresh_entry = ET.SubElement(root, "item", attrib = {"label":u"aktualisieren"})
        ...
        if osd_on != 0:
            if state == 0:
                state_entry = ET.SubElement(root, "item", attrib = {"label":"osd anschalten"})
                ...
                state_entry = ET.SubElement(root, "item", attrib = {"label":"osd abschalten"})
                state_entry_action = ET.SubElement(state_entry, "action", attrib = {"name":"Execute"})
                state_entry_execute = ET.SubElement(state_entry_action, "command")
                state_entry_execute.text = "%s --osdtoggle" % (sys.argv[0])

        open_entry = ET.SubElement(root, "item", attrib = {"label":u"mails öffnen"})
